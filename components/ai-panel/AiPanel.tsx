...
import { collectTouchedNodeIds, getUndoOperationId } from "./messages";
...
function AiPanel({ onCollapse }: { onCollapse: () => void }) {
  ...
  const handleTurnFinished = useEventCallback(
    (message: Parameters<typeof collectTouchedNodeIds>[0]) => {
      ...
      if (getUndoOperationId(message) !== null) {
        ...
        // Refreshes server-rendered chrome such as the map name in the header.
        router.refresh();
      }
    }
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 border-b border-border px-3 pb-2.5 pt-16">
        ...
        {chat.isStreaming ? (
          <span
            aria-label="Sprig is working"
            className="sprig-bloom-dot"
            role="status"
          />
        ) : null}
        <Button
      ...
      <div className="flex flex-col gap-2 border-t border-border p-3">
        {chat.error === undefined ? null : (
          <div
            className="flex items-start gap-2 rounded-md border border-destructive px-2.5 py-2 text-xs text-destructive"
            role="alert"
          >
        ...
        {isCanvasStale ? (
          <div
            className="flex items-start gap-2 rounded-md border border-line-strong bg-secondary px-2.5 py-2 text-xs text-muted-foreground"
            role="status"
          >
            <span className="flex-1">
