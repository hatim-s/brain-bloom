...
- AI: `CLAUDE_CODE_OAUTH_TOKEN` (personal Claude Code subscription token) —
  dev and single-operator only. No API-key path exists in
  `lib/claude-agent.ts` yet.
...
`lib/claude-agent.ts` authenticates ONLY via `CLAUDE_CODE_OAUTH_TOKEN`
(personal subscription OAuth generated by `claude setup-token`). That is not a
production or multi-user credential.
