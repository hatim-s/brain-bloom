...
| `benchmark:embeddings` | Validates the pinned Phase 0 local-embedding candidates and committed evaluation fixtures, then reports offline-cache readiness without loading models or writing output. | Use the default dry run during review. A real benchmark requires `--run --adapter-module <local path>`; add `--allow-downloads` only with explicit human approval when a cache is absent. |
