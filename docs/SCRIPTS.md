...
| `start` | Starts the Next.js production server using an existing build. | Use to run the output of `bun run build`. |
...
| `migrate:supabase` | Previews the Supabase-to-Convex mindmap migration without writing by default. Pass `--execute --owner <Clerk subject>` after `--` to perform the import for one owner. | Run `bun run migrate:supabase` for a dry-run report. After human approval, run `bun run migrate:supabase -- --execute --owner <Clerk subject>` to write to Convex. |
| `provider:spike` | Runs the disposable Phase 0 Codex/Claude provider-seam harness through Bun's native TypeScript execution. It prints help or a dry-run plan unless `--execute` is supplied. | Use only for human-invoked credential ceremony proofs described in `docs/PROVIDER_SPIKE.md`; ordinary tests use injected fakes. |
...
- `bun run convex:dev` — run the Convex development deployment (anonymous local by
  default; see `docs/ENV.md`).
- `bun run convex:codegen` — regenerate `convex/_generated/` from the current
...
- `bun run test:convex` — run only the `convex/**/*.test.ts` suites.

`bun run test` also includes the Convex Vitest suites.

## Supabase migration

`bun run migrate:supabase` is always a dry run unless `--execute` is present. An
executing run also requires an explicit Clerk subject:

```sh
bun run migrate:supabase -- --execute --owner user_123
```
