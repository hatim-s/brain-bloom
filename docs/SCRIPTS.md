...
| `benchmark:embeddings` | Validates pinned model/adapter/runtime/preprocessing identities and committed evaluation fixtures, then reports cache-entry existence from metadata only; it never opens cache contents, loads models, or writes output by default. | Use the default dry run during review. A real benchmark requires `--run --adapter-module <local path>`; add `--allow-downloads` only with explicit human approval when a cache is absent. `--output-dir` accepts only a relative subdirectory beneath the dedicated benchmark results root. |
